...
export function ResourceFilters({ 
  ...
  return (
    <div className={`flex flex-wrap items-center gap-4 bg-card p-4 rounded-xl border border-border transition-opacity ${isPending ? "opacity-70" : "opacity-100"}`}>
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
        ...
        >
          ...
          {subjects?.map(subject => {
            const abbreviation = subject.name.split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 4);
            return (
