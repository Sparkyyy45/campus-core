        ...
        <form action={handleCreate} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              id="description"
              name="description"
              placeholder="What should a student achieve?"
              rows={3}
            />
          </div>
          <div className="space-y-1.5">
        ...
        {Object.keys(grouped).length === 0 ? (
          ...
          Object.entries(grouped).map(([groupKey, items]) => (
            <div key={groupKey} className="campus-card">
              <h3 className="font-semibold text-xs text-muted-foreground uppercase tracking-widest mb-3">
                {groupKey}
              </h3>
              <div className="space-y-2">
                {items.map((r, idx) => (
                  <div
                    key={r.id}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-border bg-background hover:bg-muted/20 transition-colors"
                  >
                    <div className="flex flex-col gap-0.5">
                      <button
                        onClick={() => handleReorder(r.id, "up", r.order_idx)}
                        disabled={isPending || idx === 0}
                        aria-label="Move roadmap item up"
                      ...
                      <button
                        ...
                        aria-label="Move roadmap item down"
                        className="p-0.5 rounded text-muted-foreground hover:text-foreground disabled:opacity-30"
                      >
