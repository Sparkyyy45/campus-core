...
export default async function AdminPage() {
  ...
  const [
    ...
    db.from("resources").select("*", { count: "exact", head: true }).eq("status", "PUBLISHED"),
    db.from("resources").select("*", { count: "exact", head: true }).eq("status", "DRAFT"),
  ...
  if (topResourceIds.length > 0) {
    const { data: resourceRows } = await db
      .from("resources")
      .select("id, title")
      .in("id", topResourceIds.map(([id]) => id)) as {
  ...
  const primaryStats = [
    { label: "Students", value: String(studentCount ?? 0), icon: Users, color: "text-blue-600", bg: "bg-blue-50", href: "/admin/users" },
    { label: "Published Resources", value: String(resourceCount ?? 0), icon: BookOpen, color: "text-teal-600", bg: "bg-teal-50", href: "/admin/resources" },
    { label: "Total Downloads", value: String(downloadCount ?? 0), icon: Download, color: "text-purple-600", bg: "bg-purple-50", href: "/admin/resources" },
    { label: "Announcements", value: String(announcementCount ?? 0), icon: Bell, color: "text-amber-600", bg: "bg-amber-50", href: "/admin/announcements" },
  ];

  const secondaryStats = [
    { label: "7-Day Active Users", value: String(activeUsers7d), icon: Activity, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Downloads (7 Days)", value: String(downloads7d ?? 0), icon: Calendar, color: "text-indigo-600", bg: "bg-indigo-50" },
    { label: "Draft Resources", value: String(draftCount ?? 0), icon: BookOpen, color: "text-orange-600", bg: "bg-orange-50" },
    { label: "Est. Storage Used", value: storageLabel, icon: HardDrive, color: "text-rose-600", bg: "bg-rose-50" },
  ];

  const quickLinks = [
    { label: "Upload Resource", href: "/admin/resources", icon: BookOpen, desc: "Add notes, PYQs, and more" },
    { label: "Post Announcement", href: "/admin/announcements", icon: Bell, desc: "Broadcast to all students" },
    { label: "Manage Subjects", href: "/admin/subjects", icon: GraduationCap, desc: "Add or remove subjects" },
    { label: "Add Roadmap", href: "/admin/roadmaps", icon: Map, desc: "Create learning paths" },
    { label: "Manage Users", href: "/admin/users", icon: Users, desc: "View users and edit roles" },
  ];
    ...
    <div className="max-w-6xl mx-auto space-y-10">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-primary uppercase tracking-widest">Admin Control Center</span>
          </div>
        ...
        <div className="hidden sm:flex items-center gap-2 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-muted">
            <Users className="h-3 w-3" /> {adminCount ?? 0} admin{(adminCount ?? 0) !== 1 ? "s" : ""}
          </span>
          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-muted">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {primaryStats.map((stat) => (
          <Link key={stat.label} href={stat.href} className="campus-card flex items-center gap-4 hover:border-primary/20 transition-colors group">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {secondaryStats.map((stat) => (
            <div key={stat.label} className="campus-card flex items-center gap-3">
