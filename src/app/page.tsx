...
import { ArrowRight, LayoutTemplate, ShieldCheck, Map, FileText, CheckCircle2, Sparkles, Layers } from "lucide-react";
    ...
    "@graph": [
      {
        ...
        "description": "Sir Padampat Singhania University official student academic hub and resource portal.",
        "publisher": {
      ...
      },
      {
        ...
        "sameAs": [
          "https://www.spsu.ac.in",
          "https://github.com/campuscore"
        ]
      }
    ]
  };
        ...
        <div className="container mx-auto px-6 h-16 flex items-center justify-between max-w-7xl">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-xl bg-foreground text-background flex items-center justify-center shadow-inner group transition-transform hover:scale-105">
              <span className="text-sm font-bold font-serif tracking-tighter">C</span>
            </div>
            <div className="flex flex-col">
              <span className="text-base font-bold tracking-tight leading-none">CampusCore</span>
              <span className="text-[10px] text-primary font-mono tracking-wider font-semibold uppercase mt-0.5">Workspace</span>
            </div>
          </div>
          <div className="flex items-center gap-4">
            {user ? (
              <Link href="/dashboard">
                <Button size="sm" className="h-9 rounded-xl px-4 shadow-sm font-medium hover:scale-[1.02] transition-all">
                  Go to Dashboard
                </Button>
              </Link>
            ) : (
              <div className="flex items-center gap-1 sm:gap-2">
                ...
                <Link href="/signup">
                  <Button size="sm" className="h-9 rounded-xl px-4 shadow-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90 hover:scale-[1.02] transition-all">
                    Get CampusCore free
                  </Button>
                </Link>
              </div>
      ...
      <main className="flex-1">
        ...
        <section className="px-6 pt-20 pb-16 md:pt-32 md:pb-24 max-w-6xl mx-auto text-center relative">
          ...
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-muted/60 border border-border/80 text-xs font-medium text-muted-foreground mb-8 animate-fade-in-up">
            ...
            <span className="text-foreground font-semibold">Production Polish Release</span>
            <span className="text-border">|</span>
            <span>Notion-Inspired Architecture</span>
          </div>

          <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold tracking-tight text-balance leading-[1.05] mb-6 animate-fade-in-up" style={{ animationDelay: "0.05s" }}>
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto text-balance leading-relaxed mb-10 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
            CampusCore is the connected workspace where smart universities centralize verified notes, curated roadmaps, and priority announcements. 
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-3.5 animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
            <Link href={user ? "/dashboard" : "/signup"} className="w-full sm:w-auto">
              <Button size="lg" className="w-full sm:w-auto h-12 px-8 rounded-xl text-base shadow-lg shadow-primary/10 hover:scale-[1.02] transition-all duration-200 font-medium">
                Get CampusCore free <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            </Link>
            <Link href={user ? "/admin" : "/login?type=admin"} className="w-full sm:w-auto">
              <Button size="lg" variant="outline" className="w-full sm:w-auto h-12 px-8 rounded-xl text-base hover:bg-muted/50 border-border/80 transition-all duration-200">
                <ShieldCheck className="mr-2 h-4 w-4 text-primary" /> Admin Portal
              </Button>
            </Link>
          </div>

          {/* Subtle Trust Indicators */}
          <div className="mt-12 flex flex-wrap items-center justify-center gap-6 sm:gap-12 text-xs font-medium text-muted-foreground/80 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            <div className="flex items-center gap-1.5">
              <CheckCircle2 className="h-3.5 w-3.5 text-primary" /> <span>Role-Based Authorization</span>
            </div>
            <div className="flex items-center gap-1.5">
              <CheckCircle2 className="h-3.5 w-3.5 text-primary" /> <span>Cloudinary SDK Synced</span>
            </div>
            <div className="flex items-center gap-1.5">
              <CheckCircle2 className="h-3.5 w-3.5 text-primary" /> <span>Next.js 16 Turbo</span>
            </div>
          </div>
        ...
        <section className="border-t border-border/60 bg-muted/10 py-24 px-6 relative">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="text-center mb-16 space-y-3">
              <div className="text-xs font-bold text-primary tracking-widest uppercase">Uncompromised Clarity</div>
              <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-balance">Everything you need. Nothing you don&apos;t.</h2>
              <p className="text-muted-foreground text-lg max-w-2xl mx-auto text-balance">
                Built specifically for high-performing engineering students to eliminate fragmented Google Drives and chat group noise.
              </p>
            </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-16">
            <div className="flex-1 space-y-6">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-semibold border border-primary/20">
                <LayoutTemplate className="h-3.5 w-3.5" /> Synchronous Architecture
              </div>
              <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-balance leading-tight">
                Engineered for continuous productivity.
              </h2>
              <p className="text-base text-muted-foreground leading-relaxed">
                CampusCore relies on highly robust typed structures, ensuring smooth offline capability simulation and zero layout shifts. Every file uploaded undergoes signature matching before being exposed to student views.
              </p>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                {[
                  { title: "Sliding Rate Limiter", desc: "Protects sensitive server endpoints." },
                  { title: "Locked Academic Tiers", desc: "Prevents credential tampering." },
                  { title: "Responsive Collapsing", desc: "Flawless view on any mobile device." },
                  { title: "Turbopack Bundled", desc: "Optimized server responses." }
                ].map((item, idx) => (
            ...
            <div className="flex-1 w-full relative">
              ...
              <div className="bg-card border border-border rounded-2xl p-8 shadow-xl relative z-10 space-y-6">
                <div className="flex items-center justify-between pb-4 border-b border-border/60">
                  <div className="text-xs font-bold font-mono">SYSTEM_DIAGNOSTIC</div>
                ...
                <div className="space-y-4 font-mono text-xs">
                  <div className="p-3 rounded-xl bg-muted/40 border border-border flex items-center justify-between">
                    ...
                    <span className="text-emerald-600 font-semibold">Active Session Middleware</span>
                  </div>
                  <div className="p-3 rounded-xl bg-muted/40 border border-border flex items-center justify-between">
                    <span className="text-muted-foreground">Admin Rate Limit Map</span>
                    <span className="text-foreground">Initialized Module Scope</span>
                  </div>
                  <div className="p-3 rounded-xl bg-muted/40 border border-border flex items-center justify-between">
                    <span className="text-muted-foreground">SheetTrigger Typings</span>
                    <span className="text-primary font-semibold">Render Prop Resolved</span>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-primary/5 border border-primary/10 flex items-center gap-3">
                  ...
                  <p className="text-[11px] text-muted-foreground leading-relaxed font-sans">
                    Fully compatible with standard server actions and isolated layout boundaries.
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="border-t border-border/60 py-24 px-6 bg-foreground text-background text-center relative overflow-hidden">
          ...
          <div className="max-w-3xl mx-auto space-y-8 relative z-10">
            <div className="h-14 w-14 rounded-2xl bg-background text-foreground flex items-center justify-center mx-auto shadow-2xl">
              <span className="text-2xl font-bold font-serif tracking-tighter">C</span>
            </div>
            
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-balance">
              Join your campus workspace today.
            </h2>
            
            <p className="text-lg md:text-xl text-muted-foreground max-w-xl mx-auto text-balance">
              Get immediate, authorized access to verified resources, structured roadmaps, and official bulletins.
            </p>
            
            <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/signup">
                <Button size="lg" className="h-13 px-10 rounded-xl text-base font-semibold bg-background text-foreground hover:bg-muted shadow-xl hover:scale-[1.02] transition-all duration-200">
                  Register Student Account
                </Button>
              </Link>
              <Link href="/login">
                <Button size="lg" variant="ghost" className="h-13 px-6 rounded-xl text-base text-muted-foreground hover:text-background hover:bg-white/10 transition-all duration-200">
                  Sign in existing
                </Button>
              </Link>
            </div>
      ...
      <footer className="border-t border-border/40 py-12 px-6 bg-background">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <p className="text-xs text-muted-foreground font-mono">
            © {new Date().getFullYear()} CampusCore Systems. Secured by Supabase.
          </p>
        </div>
      </footer>
    </div>
